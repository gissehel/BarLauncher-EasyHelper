...
namespace BarLauncher.EasyHelper.Test.Mock.Service
{
    ...
    {
        public void OpenUrl(string url) => UrlOpened.Add(url);

        public void StartCommandLine(string command, string arguments) => CommandLineStarted.Add(new CommandLineMock(command, arguments));

        public void CopyTextToClipboard(string text) => TextCopiedToClipboard.Add(text);

        public void OpenDirectory(string directory) => StartCommandLine(directory, "");
